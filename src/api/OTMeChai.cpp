...
OTMeChai::OTMeChai(OT_ME* pMe) : m_pMe(pMe)
{
}
...
bool OTMeChai::ExecuteScript_ReturnBool(const OTString& code,
                                        const char* pFunction)
{
    return m_pMe->ExecuteScript_ReturnBool(code.Get(), pFunction);
}

int32_t OTMeChai::ExecuteScript_ReturnInt(const OTString& code,
                                          const char* pFunction)
{
    return m_pMe->ExecuteScript_ReturnInt(code.Get(), pFunction);
}

std::string OTMeChai::ExecuteScript_ReturnString(const OTString& code,
                                                 const char* pFunction)
{
    return m_pMe->ExecuteScript_ReturnString(code.Get(), pFunction);
}

void OTMeChai::ExecuteScript_ReturnVoid(const OTString& code,
                                        const char* pFunction)
{
...
{
    OTString code;
    code.Format("{ var bResult = OT_ME_insure_enough_nums(int32_t(%" PRId32
                "), \"%s\", \"%s\"); }",
                                       ...
                                       const std::string& ALT_LOCATION)
{
    ...
    AddVariable(varAlt);
    OTString code;
    code.Format("{ var strResult = OT_ME_create_pseudonym(int32_t(%" PRId32
                "), %s, %s); }",
                                       ...
                                       const std::string& THE_CONTRACT)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_issue_asset_type(\"%s\", \"%s\", %s); }",
                                            ...
                                            const std::string& THE_BASKET)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_issue_basket_currency(\"%s\", \"%s\", %s); }",
        SERVER_ID.c_str(), NYM_ID.c_str(), str_var_name1.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                               ...
                                               const bool IN_OR_OUT)
{
    ...
    AddVariable(varTheContract);
    OTString code;
    code.Format("{ var strResult = OT_ME_exchange_basket_currency(\"%s\", "
                "\"%s\", \"%s\", %s, \"%s\", %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ASSET_TYPE_ID.c_str(),
                str_var_name1.c_str(), ACCOUNT_ID.c_str(),
                IN_OR_OUT ? "true" : "false");
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                        ...
                                        const std::string& CONTRACT_ID)
{
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_retrieve_contract(\"%s\", \"%s\", \"%s\"); }",
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_load_or_retrieve_contract(\"%s\", "
                "\"%s\", \"%s\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), CONTRACT_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                        ...
                                        const std::string& ASSET_TYPE_ID)
{
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_create_asset_acct(\"%s\", \"%s\", \"%s\"); }",
        SERVER_ID.c_str(), NYM_ID.c_str(), ASSET_TYPE_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_retrieve_account(\"%s\", \"%s\", "
                "\"%s\", %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ACCOUNT_ID.c_str(),
                bForceDownload ? "true" : "false");
    return ExecuteScript_ReturnBool(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var bWasSent = OTBool(false); var bResult = "
                "details_refresh_nym(\"%s\", \"%s\", bWasSent, %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(),
                bForceDownload ? "true" : "false");
    return ExecuteScript_ReturnBool(code, __FUNCTION__);
}
                                    ...
                                    const std::string& NOTE)
{
    ...
    AddVariable(varNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_send_transfer(\"%s\", \"%s\", \"%s\", "
                "\"%s\", int64_t(%" PRId64 "), %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ACCT_FROM.c_str(),
                ACCT_TO.c_str(), AMOUNT, str_var_name1.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                    ...
                                    const std::string& RESPONSE_LEDGER)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_process_inbox(\"%s\", \"%s\", \"%s\", %s); }",
        SERVER_ID.c_str(), NYM_ID.c_str(), ACCOUNT_ID.c_str(),
        str_var_name.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var nResult = accept_inbox_items(\"%s\", int32_t(%" PRId32
                "), \"%s\"); var bResult = ((1 == nResult) ? true : false); }",
...
{
    OTString code;
    code.Format("{ var nResult = details_discard_incoming(\"%s\", \"%s\", "
                "\"%s\"); var bResult = ((1 == nResult) ? true : false); }",
...
{
    OTString code;
    code.Format("{ var nResult = details_cancel_outgoing(\"%s\", \"%s\", "
                "\"%s\"); var bResult = ((1 == nResult) ? true : false); }",
                                         ...
                                         const std::string& PAYMENT_TYPE)
{
    OTString code;
    code.Format(
        "{ var nResult = accept_from_paymentbox(\"%s\", \"%s\", \"%s\"); }",
...
std::string OTMeChai::load_or_retrieve_encrypt_key(
    const std::string& SERVER_ID, const std::string& NYM_ID,
    const std::string& TARGET_NYM_ID)
{
    OTString code;
    code.Format("{ var strResult = OT_ME_load_or_retrieve_encrypt_key(\"%s\", "
                "\"%s\", \"%s\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), TARGET_NYM_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}

std::string OTMeChai::load_or_retrieve_signing_key(
    const std::string& SERVER_ID, const std::string& NYM_ID,
    const std::string& TARGET_NYM_ID)
{
    OTString code;
    code.Format("{ var strResult = OT_ME_load_or_retrieve_signing_key(\"%s\", "
                "\"%s\", \"%s\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), TARGET_NYM_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                           ...
                                           const std::string& THE_MESSAGE)
{
    ...
    AddVariable(varNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_send_user_msg_pubkey(\"%s\", \"%s\", "
                "\"%s\", %s, %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), RECIPIENT_NYM_ID.c_str(),
                str_var_name1.c_str(), str_var_name2.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                            ...
                                            const std::string& THE_INSTRUMENT)
{
    ...
    AddVariable(varNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_send_user_pmnt_pubkey(\"%s\", \"%s\", "
                "\"%s\", %s, %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), RECIPIENT_NYM_ID.c_str(),
                str_var_name1.c_str(), str_var_name2.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
...
std::string OTMeChai::send_user_cash_pubkey(
    const std::string& SERVER_ID, const std::string& NYM_ID,
    ...
    const std::string& THE_INSTRUMENT, const std::string& INSTRUMENT_FOR_SENDER)
{
    ...
    OTString code;
    code.Format("{ var strResult = OT_ME_send_user_cash_pubkey(\"%s\", \"%s\", "
                "\"%s\", %s, %s, %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), RECIPIENT_NYM_ID.c_str(),
                str_var_name1.c_str(), str_var_name2.c_str(),
                str_var_name3.c_str());
                                    ...
                                    const std::string& THE_MESSAGE)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_send_user_msg(\"%s\", \"%s\", \"%s\", %s); }",
        SERVER_ID.c_str(), NYM_ID.c_str(), RECIPIENT_NYM_ID.c_str(),
        str_var_name.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                        ...
                                        const std::string& THE_PAYMENT)
{
    ...
    AddVariable(varNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_send_user_payment(\"%s\", \"%s\", "
                "\"%s\", %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), RECIPIENT_NYM_ID.c_str(),
                str_var_name.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
                                     ...
                                     const std::string& SENDERS_COPY)
{
    ...
    AddVariable(varSenderNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_send_user_cash(\"%s\", \"%s\", "
                "\"%s\", %s, %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), RECIPIENT_NYM_ID.c_str(),
                str_var_name.c_str(), str_var_name2.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                      ...
                                      const int64_t AMOUNT)
{
    ...
    OTString code;
    code.Format(
        "{ var bResult = withdraw_and_send_cash(\"%s\", \"%s\", %s, \"%" PRId64
        "\"); }",
        ACCT_ID.c_str(), RECIPIENT_NYM_ID.c_str(), str_var_name.c_str(),
        AMOUNT);
    return ExecuteScript_ReturnBool(code, __FUNCTION__);
}
                                             ...
                                             const std::string& PRELOADED_INBOX)
{
    ...
    AddVariable(varBox);
    OTString code;
    code.Format("{ var strResult = OT_ME_get_payment_instrument(\"%s\", "
                "\"%s\", int32_t(%" PRId32 "), %s); }",
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_get_box_receipt(\"%s\", \"%s\", "
                "\"%s\", int32_t(%" PRId32 "), \"%" PRId64 "\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ACCT_ID.c_str(), nBoxType,
                TRANS_NUM);
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                    ...
                                    const std::string& ASSET_ID)
{
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_retrieve_mint(\"%s\", \"%s\", \"%s\"); }",
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_load_or_retrieve_mint(\"%s\", \"%s\", "
                "\"%s\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ASSET_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                        ...
                                        const std::string& ENCODED_MAP)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_query_asset_types(\"%s\", \"%s\", %s); }",
        SERVER_ID.c_str(), NYM_ID.c_str(), str_var_name.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}

std::string OTMeChai::create_market_offer(
    ...
    const std::string& STOP_SIGN, const int64_t ACTIVATION_PRICE)
{
    ...
    code.Format("{ var strResult = OT_ME_create_market_offer(\"%s\", \"%s\","
                ...
                "\", %s, \"%" PRId64 "\", \"%s\", \"%" PRId64 "\"); }",
                ASSET_ACCT_ID.c_str(), CURRENCY_ACCT_ID.c_str(), scale,
                minIncrement, quantity, price, bSelling ? "true" : "false",
                lLifespanInSeconds, STOP_SIGN.c_str(), ACTIVATION_PRICE);
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_kill_market_offer(\"%s\", \"%s\", "
                "\"%s\", \"%" PRId64 "\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ASSET_ACCT_ID.c_str(),
                TRANS_NUM);
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_kill_payment_plan(\"%s\", \"%s\", "
                "\"%s\", \"%" PRId64 "\"); }",
                                          ...
                                          const std::string& THE_PAYMENT_PLAN)
{
    ...
    OTString code;
    code.Format("{ var strResult = OT_ME_cancel_payment_plan(\"%s\", \"%s\", "
                "varContract); }",
                SERVER_ID.c_str(), NYM_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}

std::string OTMeChai::activate_smart_contract(
    const std::string& SERVER_ID, const std::string& NYM_ID,
    ...
    const std::string& THE_SMART_CONTRACT)
{
    ...
    AddVariable(varContract);
    OTString code;
    code.Format("{ var strResult = OT_ME_activate_smart_contract(\"%s\", "
                "\"%s\", \"%s\", \"%s\", %s); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ACCT_ID.c_str(),
                AGENT_NAME.c_str(), str_var_name.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                     ...
                                     const std::string& STR_PARAM)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_trigger_clause(\"%s\", \"%s\", \"%" PRId64
        "\", \"%s\", %s); }",
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_withdraw_cash(\"%s\", \"%s\", \"%s\", "
                "int64_t(%" PRId64 ")); }",
                                  ...
                                  std::string& STR_RETAINED_COPY)
{
    ...
    OTString code;
    code.Format("{ var strResult = details_export_cash(\"%s\", \"%s\", \"%s\", "
                "\"%s\", \"%s\", %s, %s); }",
                SERVER_ID.c_str(), FROM_NYM_ID.c_str(), ASSET_TYPE_ID.c_str(),
                TO_NYM_ID.c_str(), STR_INDICES.c_str(),
                bPasswordProtected ? "true" : "false", str_var_name1.c_str());
    const std::string str_result =
        ExecuteScript_ReturnString(code, __FUNCTION__);

    if (!str_result.empty()) STR_RETAINED_COPY = varRetained.GetValueString();
                                       ...
                                       const int64_t AMOUNT)
{
    ...
    AddVariable(varNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_withdraw_voucher(\"%s\", \"%s\", "
                "\"%s\", \"%s\", %s, int64_t(%" PRId64 ")); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), ACCT_ID.c_str(),
                RECIP_NYM_ID.c_str(), str_var_name1.c_str(), AMOUNT);
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                   ...
                                   const int64_t AMOUNT_PER_SHARE)
{
    ...
    AddVariable(varNote);
    OTString code;
    code.Format("{ var strResult = OT_ME_pay_dividend(\"%s\", \"%s\", \"%s\", "
                "\"%s\", %s, int64_t(%" PRId64 ")); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), SOURCE_ACCT_ID.c_str(),
                SHARES_ASSET_ID.c_str(), str_var_name1.c_str(),
                AMOUNT_PER_SHARE);
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                                     ...
                                     const std::string& STR_CHEQUE)
{
    ...
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_deposit_cheque(\"%s\", \"%s\", \"%s\", %s); }",
        SERVER_ID.c_str(), NYM_ID.c_str(), ACCT_ID.c_str(),
        str_var_name1.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
                               ...
                               const std::string& STR_PURSE)
{
    ...
    code.Format("{ details_deposit_purse(\"%s\", \"%s\", \"%s\", %s, \"\"); }",
                SERVER_ID.c_str(), ACCT_ID.c_str(), NYM_ID.c_str(),
                str_var_name1.c_str());
    return ExecuteScript_ReturnInt(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_get_market_offers(\"%s\", \"%s\", "
                "\"%s\", int64_t(%" PRId64 ")); }",
                                            ...
                                            const std::string& NYM_ID)
{
    OTString code;
    code.Format(
        "{ var strResult = OT_ME_get_nym_market_offers(\"%s\", \"%s\"); }",
        SERVER_ID.c_str(), NYM_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_get_market_recent_trades(\"%s\", "
                "\"%s\", \"%s\"); }",
                SERVER_ID.c_str(), NYM_ID.c_str(), MARKET_ID.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
...
{
    OTString code;
    code.Format("{ var strResult = OT_ME_adjust_usage_credits(\"%s\", \"%s\", "
                "\"%s\", \"%s\"); }",
                SERVER_ID.c_str(), USER_NYM_ID.c_str(), TARGET_NYM_ID.c_str(),
                ADJUSTMENT.c_str());
    return ExecuteScript_ReturnString(code, __FUNCTION__);
}
