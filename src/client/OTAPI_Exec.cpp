...
std::string OTAPI_Exec::CreateNymLegacy(
    ...
    [[maybe_unused]] const std::string& NYM_ID_SOURCE) const  // Can be empty.
{
    auto reason = api_.Factory().PasswordPrompt("Creating a new nym");
    ...
    Nym_p pNym = api_.Wallet().Nym(*nymParameters, reason);
    ...
    const std::int32_t index) const
{
    ...
    auto nym = api_.Wallet().Nym(nymParameters, reason, type, name);
    ...
    const std::string& PAYMENT_PLAN) const
{
    ...
    if (!strPlan->Exists() ||
        (false == thePlan->LoadContractFromString(strPlan, reason))) {
    ...
    const std::string& THE_CONTRACT) const  // true or false?
{
    ...
    auto pScriptable(api_.Factory().Scriptable(strContract, reason));
    if (false == bool(pScriptable)) {
        ...
        const bool bVerified =
            pScriptable->VerifyThisAgainstAllPartiesSignedCopies(reason);
        if (!bConfirmed) {
    ...
    const std::string& PARTY_NAME) const
{
    ...
    std::unique_ptr<OTScriptable> pScriptable(
        api_.Factory().Scriptable(strContract, reason));
    if (false == bool(pScriptable)) {
        LogNormal(OT_METHOD)(__FUNCTION__)(
            ": Failed trying to load smart contract from string: ")(
            strContract)(".")
    ...
    const std::string& ASSET_ACCT_ID) const
{
    ...
    if (theBasket->LoadContractFromString(strBasket, reason)) {
