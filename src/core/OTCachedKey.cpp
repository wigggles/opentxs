...
_SharedPtr<OTCachedKey> OTCachedKey::It(OTIdentifier* pIdentifier /*=NULL*/)
{
    ...
    //
    static _SharedPtr<OTCachedKey> s_theSingleton(
        new OTCachedKey); // Default is 0 ("you have to type your PW a million
                          // times"), but it's overridden in config file.

    if (NULL == pIdentifier)
        return s_theSingleton; // Notice if you pass NULL (no args) then it
...
//
// static
_SharedPtr<OTCachedKey> OTCachedKey::It(OTCachedKey& theSourceKey) // Note:
                                                                   // parameter
                                                                   // no longer
                                                                   // const due
                                                                   // to need to
                                                                   ...
                                                                   // mutex.
{
    ...
    // By this point, pMaster is definitely NULL. (Not found on the map, needs
    // to be added.)
    //
    ...
    {
        _SharedPtr<OTCachedKey> pMaster(
            new OTCachedKey); // int32_t nTimeoutSeconds=OT_MASTER_KEY_TIMEOUT;

        pMaster->SetCachedKey(ascCachedKey);

        s_mapCachedKeys.insert(std::pair<std::string, _SharedPtr<OTCachedKey>>(
...
OTCachedKey::~OTCachedKey()
{
    ...
    if (NULL != m_pMasterPassword) // Only stored temporarily, the purpose of
    ...
    if (NULL != m_pSymmetricKey) // Owned / based on a string passed in. Stored
...
bool OTCachedKey::SerializeFrom(const OTASCIIArmor& ascInput)
{
    ...
    if (NULL == m_pSymmetricKey) return false;
...
// Caller must delete!
// static
_SharedPtr<OTCachedKey> OTCachedKey::CreateMasterPassword(
                                    ...
                                    bool bVerifyTwice /*=false*/)
{
    ...
    std::string str_display(NULL != szDisplay ? szDisplay
                                              : "(Display string was blank.)");

    const char* szFunc = "OTCachedKey::GetMasterPassword";

    //  OT_ASSERT(NULL != m_pSymmetricKey); // (This had better be set already.)
    ...
    */
    ...
    // return
    // a simple 1 or 0 (instead of the length.) buf and size can be NULL / 0,
    // and
    ...
    { // then we might have also already stashed the derived key on the system
        ...
        if (bFoundOnKeyring) // We found it -- but does it WORK?
        {
            ...
            else                 // It didn't unlock with the one we found.
            {
                ...
                delete pDerivedKey;
                pDerivedKey =
                    NULL; // Below, this function checks pDerivedKey for NULL.
            }
        }
    ...
    //
    if (NULL == pDerivedKey) // Master key was not cached in OT, nor was it
    ...
    { // Therefore we HAVE to ask the user for a passphrase and decrypt it
        ...
        if (!bGenerated) // This Symmetric Key hasn't been generated before....
        {
            ...
            if (NULL != pDerivedKey)
                ...
                otErr << __FUNCTION__ << ": FYI: Derived key is still NULL "
                                         "after calling "
               ...
               // already generated.)
        {
            ...
            if (m_pSymmetricKey->HasHashCheck()) {
                ...
                if (!OTAsymmetricKey::GetPasswordCallback()(
                         NULL, 0, true, static_cast<void*>(&thePWData))) {
                    otErr << __FUNCTION__
                          << ": Failed to get password from user!";
                    return false;
                }
        ...
        if (bGenerated) // If SymmetricKey (*this) is already generated.
        {
            ...
            // be not-NULL, and will be used here, and will be used subsequently
            // for adding to the system
            // keychain. Otherwise, it will be NULL, and GetRawKeyFromPassphrase
            // will thus just derive its
            // own copy of the derived key internally. It will still work, but
            // then back up here, it will
            // NOT be added to the system keyring, since it's still NULL back up
            // here.
            // (FYI.)
            //
...
// from RAM.\n", szFunc);
        ...
        _SharedPtr<OTCachedKey>* pthreadSharedPtr =
            new _SharedPtr<OTCachedKey>(mySharedPtr); // TODO: memory leak.
...
void OTCachedKey::ThreadTimeout(void* pArg)
{
    ...
    //
    _SharedPtr<OTCachedKey>* pthreadSharedPtr =
        static_cast<_SharedPtr<OTCachedKey>*>(pArg);
...
void OTCachedKey::DestroyMasterPassword()
{
    ...
    // what CALLED this function. Instead, we destroy / NULL the master
    // password,
