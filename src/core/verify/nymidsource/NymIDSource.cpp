    ...
    const proto::SourceType type)
{
    if (!serializedNymIDSource.has_version()) {
        otErr << "Verify serialized nym source failed: missing version.\n";
        return false;
    }
    ...
    if ((version < minVersion) || (version > maxVersion)) {
        otErr << "Verify serialized nym source failed: incorrect version ("
              << serializedNymIDSource.version() << ").\n";
        return false;
    }
    ...
    switch (version) {
        ...
        default :
            otErr << "Verify serialized nym source failed: unknown version ("
                  << serializedNymIDSource.version() << ").\n";
