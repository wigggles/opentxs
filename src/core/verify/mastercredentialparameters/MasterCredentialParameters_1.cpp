    ...
    const proto::SourceType sourceType)
{
    ...
    if (!validSource) {
        otErr << "Verify serialized master parameters failed: invalid nym id source.\n";
        return false;
    }
    ...
    switch (sourceType) {
        case SOURCETYPE_SELF :
            if (serializedMasterParams.has_sourceproof()) {
                otErr << "Verify serialized master parameters failed: self-signed credential contains source proof.\n";
                return false;
            }
            validProof = true;
            ...
            break;
        default :
            otErr << "Verify serialized master parameters failed: incorrect or unknown source type ("
            << sourceType << ").\n";
    ...
    if (!validProof) {
        otErr << "Verify serialized master parameters failed: invalid nym id source proof.\n";
        return false;
    }
