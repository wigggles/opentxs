    ...
    const ChildCredentialParameters serializedChildParams)
{
    if (!serializedChildParams.has_masterid()) {
        otErr << "Verify serialized child parameters failed: missing master identifier.\n";
        return false;
    }

    if (MIN_PLAUSIBLE_IDENTIFIER > serializedChildParams.masterid().size()) {
        otErr << "Verify serialized credential failed: invalid master identifier ("
                << serializedChildParams.masterid() << ").\n";
        return false;
    }
