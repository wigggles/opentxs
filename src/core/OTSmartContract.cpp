...
int64_t OTSmartContract::GetClosingNumber(const OTIdentifier& theAcctID) const
{
    OTPartyAccount* pPartyAcct =
        this->GetPartyAccountByID(theAcctID); // from OTScriptable.
...
std::string OTSmartContract::GetAcctBalance(const std::string from_acct_name)
{
    ...
    //
    //
    //    if (false == this->VerifyPartyAuthorization(*pFromParty, *pServerNym,
                                             ...
                                             const std::string asset_type_id)
{
    ...
    OTStash* pStash = this->GetStash(from_stash_name); // This ALWAYS succeeds.
                                                       // (It will OT_ASSERT()
                                                       // if failure.)
...
bool OTSmartContract::SendANoticeToAllParties()
{
    ...
    if (0 == lNewTransactionNumber) {
        ...
        bDroppedNotice = this->SendNoticeToAllParties(
                                     ...
                                     const std::string str_Amount)
{
    ...
    OTPartyAccount* pFromAcct = this->GetPartyAccount(from_acct_name);
    OTStash* pStash = this->GetStash(to_stash_name); // This ALWAYS succeeds.
    ...
    bool bMoved = this->StashFunds(map_Nyms_Already_Loaded, lAmount,
                                   theFromAcctID, theFromAgentID, *pStash);
    if (!bMoved) {
                                       ...
                                       const std::string str_Amount)
{
    ...
    //
    //
    //    if (false == this->VerifyPartyAuthorization(*pToParty, *pServerNym,
    // strServerID, &map_Nyms_Already_Loaded))
    //    {
    ...
    bool bMoved = this->StashFunds(map_Nyms_Already_Loaded, lNegativeAmount,
...
//
    ...
    bool bWasAcctCreated = false; // GetOrCreateAccount() will verifyContractID
                                  ...
                                  // internally.
    pStashAccount = m_StashAccts.GetOrCreateAccount(
    ...
    if (!this->VerifyNymAsAgentForAccount(*pPartyNym, *pPartyAssetAcct)) {
        ...
        if (false == bSuccessLoadingPartyInbox) {
            ...
            // GetTransactionNum();
            const int64_t lPartyTransRefNo =
                this->GetOpeningNumber(PARTY_USER_ID);
...
// Switching to string.
{
    ...
    bool bMoved =
        this->MoveFunds(map_Nyms_Already_Loaded, lAmount, theFromAcctID,
                                                                 ...
                                                                 // param.
{
    ...
    //
    if (this->IsDirtyImportant()) // This tells us if any "Important" variables
                                  // have changed since executing the scripts.
    {
        ...
        if (0 == lNewTransactionNumber) {
            ...
            const OTString strReference(*this);
            bool bDroppedNotice = this->SendNoticeToAllParties(
                true, // bSuccessMsg=true
                *pServerNym, GetServerID(), lNewTransactionNumber,
                // GetTransactionNum(), // each party has its own opening trans
                // #.
...
bool OTSmartContract::CanRemoveItemFromCron(OTPseudonym& theNym)
{
    ...
    //
    OTAgent* pAgent = NULL;
    OTParty* pParty = this->FindPartyBasedOnNymAsAgent(
    ...
    if (bPartyHasName &&
        this->CanCancelContract(str_party_name)) // Here is where it calls the
                                          ...
                                          const bool bBurnTransNo /*=false*/)
{
    ...
    mapOfNyms map_Nyms_Already_Loaded; // The list of Nyms that were already
                                       ...
                                       // called.
    this->RetrieveNymPointers(map_Nyms_Already_Loaded); // now theNym is on this
                                                        // map. (His party
    ...
    FOR_EACH_IT(mapOfParties, m_mapParties, it_party)
    {
        ...
        const bool bIsPartyAuthorized = this->VerifyPartyAuthorization(
    ...
    //
    this->ClearTemporaryPointers(); // Don't want any bad pointers floating
...
// Then add API functions for loading/save editable and template smart contracts
// to/from local storage.

// Then add API functions for issuing and downloading the smart contracts
// to/from server.

// Then add API functions for downloading and loading up the list of INSTANCES
// (contracts activated on server.)

// The "INSTANCES" tab will show contracts not only that are activated, but also
// that are confirmed and pending activation.
...
{
    if (!strName.Exists()) {
        otErr << "OTSmartContract::LoadEditable: Empty name passed in. "
                 "(Returning false.)\n";
        return false;
    }
    ...
    if (!OTDB::Exists(OTFolders::SmartContracts().Get(), strServerID.Get(),
                      "editing", // todo hardcoding. The folder where smart
                                 // contracts are edited.
                      ascName.Get())) {
        otInfo << "OTSmartContract::LoadEditable: File doesn't exist: "
               << OTFolders::SmartContracts().Get() << OTLog::PathSeparator()
               << strServerID << OTLog::PathSeparator() << "editing"
               << OTLog::PathSeparator() << ascName << "\n";
        return false;
    }
    else
        str_contract = OTDB::QueryPlainString(
            OTFolders::SmartContracts().Get(), strServerID.Get(),
            "editing", // todo stop hardcoding.
            ascName.Get());
    if (str_contract.size() < 1) {
        otErr << "OTSmartContract::LoadEditable: Failed loading: "
              << OTFolders::SmartContracts().Get() << OTLog::PathSeparator()
              << strServerID << OTLog::PathSeparator() << "editing"
              << OTLog::PathSeparator() << ascName << "\n";
    }
        ...
        if (false == this->LoadContractFromString(
                         strSmartContract)) // Handles OT ARMORED file formats.
        {
            otErr << "OTSmartContract::LoadEditable: Failed attempt to load "
                     "(editable), smart contract from storage: " << strName
                  << " \n";
        }
        return true;
    }
...
bool OTSmartContract::SaveEditable(const OTString& strName)
{
    ...
    pList->SetValue(str_asc_name,
                    str_name); // Add the filename to the map of filenames.

    if (false == OTDB::StoreObject(*pList, OTFolders::SmartContracts().Get(),
                                   strServerID.Get(),
                                   "editing", // todo stop hardcoding.
                                   "list.dat")) {
        otErr << "OTSmartContract::SaveEditable: Failed saving "
              << OTFolders::SmartContracts() << OTLog::PathSeparator()
              << strServerID << OTLog::PathSeparator() << "editing"
    ...
    else // We saved the list of filenames -- now save the contract itself.
    {
        if (m_strRawFile.Exists()) {
            ...
            if (false ==
                ...
                otErr << "OTSmartContract::SaveEditable: Error saving editable "
                         "smart contract (failed writing armored string):\n"
                      << OTFolders::SmartContracts() << OTLog::PathSeparator()
                      << strServerID << OTLog::PathSeparator() << "editing"
                      << OTLog::PathSeparator() << str_asc_name << "\n";
                return false;
            }

            if (false == OTDB::StorePlainString(
                             strFinal.Get(), OTFolders::SmartContracts().Get(),
                             strServerID.Get(),
                             "editing",     // todo stop hardcoding.
                             str_asc_name)) // encoded version of name.
            {
                otErr << "OTSmartContract::SaveEditable: Failed saving "
                         "(editable) smart contract: " << str_name
                      << " \n to path: " << OTFolders::SmartContracts()
                      << OTLog::PathSeparator() << strServerID
                      << OTLog::PathSeparator() << "editing"
                      << OTLog::PathSeparator() << str_asc_name << "";
            }
            else {
                return true;
            ...
            otErr << "OTSmartContract::SaveEditable: Failed attempt to save "
                     "empty (unsigned?), editable, smart contract: " << str_name
                  << " \n";
        }
    }
...
// Once a smart contract is issued onto a server, then it's available there as a
// template.
...
// escrow contracts
// might be otherwise identical, except for gold in one instance, and silver in
// another.)
//
// Whether the template is on the server side or client side, this function
// behaves identically.
//
bool OTSmartContract::LoadTemplate(const OTIdentifier& SERVER_ID,
                                   const OTIdentifier& CONTRACT_ID)
{
    ...
    const char* szFolder1name =
        OTFolders::SmartContracts().Get();         // "smartcontracts"
    const char* szFolder2name = strServerID.Get(); // "smartcontracts/SERVER_ID"
    const char* szFilename =
        strContractID.Get(); // "smartcontracts/SERVER_ID/CONTRACT_ID"

    if (false == OTDB::Exists(szFolder1name, szFolder2name, szFilename)) {
        otOut << "OTSmartContract::LoadTemplate: File does not exist: "
    ...
    if (strFileContents.length() < 2) {
        otErr << "OTSmartContract::LoadTemplate: Error reading file: "
              << szFolder1name << OTLog::PathSeparator() << szFolder2name
              << OTLog::PathSeparator() << szFilename << "\n";
        return false;
    }
    ...
    bool bSuccess = LoadContractFromString(
        strRawFile); // Handles OT ARMORED file formats...
...
bool OTSmartContract::SaveTemplate(const OTIdentifier& SERVER_ID)
{
    ...
    if (!SaveContractRaw(strRawFile)) {
        otErr << "OTSmartContract::SaveTemplate: Error saving smart contract "
                 "template (to string):\n" << szFolder1name
    ...
    if (false ==
        ascTemp.WriteArmoredString(strFinal, m_strContractType.Get())) {
        otErr << "OTSmartContract::SaveTemplate: Error saving smart contract "
                 "template (failed writing armored string):\n" << szFolder1name
              << OTLog::PathSeparator() << szFolder2name
              << OTLog::PathSeparator() << szFilename << "\n";
        return false;
    }
    ...
    if (!bSaved) {
        otErr << "OTSmartContract::SaveTemplate: Error writing to file: "
              << szFolder1name << OTLog::PathSeparator() << szFolder2name
              << OTLog::PathSeparator() << szFilename << "\n";
