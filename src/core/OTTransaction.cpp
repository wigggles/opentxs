...
bool OTTransaction::VerifyAccount(OTPseudonym& theNym)
{
    ...
    // todo security audit:
    else if (IsAbbreviated() && (pParent != NULL) &&
    ...
    const bool bTransactionWasFailure) // false until positively asserted.
{
    ...
    case OTTransaction::exchangeBasket: // Uses X transaction #s: the opener,
                                        // which is burned success-or-fail, and
                                        // Y closers
        // (one for each account.) Closers are marked "used" if success
        // transaction,
        // but if message succeeds while transaction fails, then closers can be
        // harvested.
        {
            ...
            else // pItem is good. Let's load up the OTBasket object...
            {
                ...
                const bool bLoadContractFromString =
                    (strBasket.Exists() &&
                     theBasket.LoadContractFromString(strBasket));

                // If failed to load the request basket...
                if (!bLoadContractFromString) {
                    otErr << "OTTransaction::HarvestClosingNumbers: ERROR: "
                             "Failed loading basket request from string:\n\n"
                          << strBasket << "\n\n";
                }
                else // theBasket is ready to go....
                {
                    ...
                    // NOTE: if reply was failure, then I DO want to re-try my
                    // basket exchange, without having to call
                    // all the API functions to build the exchange object again
                    // (instead just re-use the existing one.)
                    //
                    // So... the closing numbers ARE all harvestable, in the
                    // event, for example, that I was out of re-tries
                    // and I just wanted to claw ALL the numbers back.
                    // But what if I just want the opening one at first, and
                    // leave the others for my retries?
                    //
                    // ===> DONE:  SOunds like I need an API call like
                    // OT_API_Msg_HarvestTransactionNumbers
                    // except called OT_API_Msg_HarvestOpeningNumber. Then it
                    // just only calls the opening version.
                    //
                    // Better: (done) Add a boolean to existing functions:
                    // "bHarvestingForRetry". The code can just be smart enough,
                    // based on that!

                    if (bReplyWasFailure && !bHarvestingForRetry) {
                        // if we WERE harvesting for a re-try, then we would
                        // LEAVE the closing numbers, so that the re-try would
                        // work, since otherwise a new basket object would need
                        // to be setup in order to make the call, with new
                        // closing numbers.
                        //
                        theBasket.HarvestClosingNumbers(
                            theNym, GetPurportedServerID(), true); // bSave=true
                        bSuccess = true;
                    ...
                    // HARVESTABLE.
                    // Why? Because that means the transaction definitely
                    // ran--and the opener for exchangeBasket is always burned,
                    // success-or-fail.
                    // The CLOSING numbers also are not harvestable on
                    // transaction success, since this marks them as "USED."
                    // ===> But the CLOSING numbers ARE harvestable on
                    // transaction *failure.*
                    //
                    else if (bReplyWasSuccess) {
                        if (bTransactionWasSuccess) {
                            // (They are not harvestable on transaction success
                            // though.)
                            // This means the "exchangeBasket" closing trans#s
                            // (one for each asset account involved in the
                            // exchange)
                            // are all MARKED AS "USED", and will someday be
                            // marked as CLOSED.
                            // EITHER WAY, you certainly can't claw those
                            // numbers back now! (They are still outstanding,
                            // though. They're not gone, yet...)
                                                         ...
                                                         // to work.
                        {
                            // But the CLOSING numbers ARE harvestable on
                            // transaction FAILURE.
                            // (Closing numbers for exchangeBasket are only
                            // marked "used" if the
                            // exchangeBasket transaction was a success. But in
                            // this case since
                            // the transaction was NOT a success, ergo they were
                            // never marked as
                            // "used," either. So let's harvest them...)
                            //
                            theBasket.HarvestClosingNumbers(
                                theNym, GetPurportedServerID(),
                                true); // bSave=true
                            bSuccess = true;
                        }
                    } // else if (bReplyWasSuccess)
...
// Client-side.
//
// This transaction actually was saved as a transaction receipt (filename:
// nymID.success)
...
//
// But what if the last receipt is a balance receipt, instead of a transaction
// receipt? Let's
// say I grab the Nym and he has only a 9! And though this transaction receipt
// shows a 9
// and 10, there is a newer balance receipt that shows only a 9? That means even
// when
// verifying a transaction receipt, I need to also load the last balance receipt
// and (for
// transaction numbers, anyway) use whichever one is newer.
//
// From there, it's simple: Make sure they match.
// I should do this when I already have the last receipt and I just grabbed a
// copy of the Nym.
//
// BUT!! I can't load the last balance receipt, since I don't have the account
// number.
// So let's say #10 disappears, as the last balance receipt would show (but
// which I can't see.)
// That means THE_NYM will show that 10 is missing, even though *this shows that
// #10 is still
// on the list. Is that bad?
//
// New transaction numbers can only be issued to my signed-out list of
// responsibility in the
// case where I SIGNED for each number using a transaction agreement.
// Whereas transaction numbers can only be REMOVED via a balance agreement
// (processInbox.)
// (NOTE: or they can be removed from an expired or canceled cron item -- aka
// finalReceipt.)
// Therefore, at most, I will see that a number has disappeared, NOT that one
// was added.
//
// Conclusion: as long as numbers only DISAPPEAR, and are not ADDED, then this
// verify is good.
// But in VerifyBalanceReceipt (next function down) I will need to load the
// transaction receipt
// and use it for verifying issued numbers, in cases where that receipt is newer
// than the balance
// receipt. (Because in that case, numbers may have been ADDED..)
//
bool OTTransaction::VerifyTransactionReceipt(
    OTPseudonym& SERVER_NYM, // For verifying a signature.
    OTPseudonym& THE_NYM)
{
    if (IsAbbreviated()) {
        otErr << "OTTransaction::VerifyTransactionReceipt: Error: This is an "
    ...
    if (NULL == pResponseBalanceItem) {
        ...
        // error, return.
        otOut << "Error: atTransactionStatement found on receipt, but not a "
                 "successful one.\n";
        return false;
    }
    else if (!pResponseBalanceItem->VerifySignature(SERVER_NYM)) {
        otOut << "Unable to verify signature on atTransactionStatement item in "
                 "OTTransaction::VerifyTransactionReceipt.\n";
        return false;
    }
    ...
    if (!strBalanceItem.Exists()) {
        // error, return.
        otOut << "No transactionStatement item found as 'in ref to' string on "
                 "a receipt containing atTransactionStatement item.\n";
        return false;
    }

    pBalanceItem =
        OTItem::CreateItemFromString(strBalanceItem, GetPurportedServerID(),
                                     pResponseBalanceItem->GetReferenceToNum());

    if (NULL == pBalanceItem) {
        otOut << "Unable to load transactionStatement item from string (from a "
                 "receipt containing an atTransactionStatement item.)\n";
        return false;
    }
        ...
        otOut << "Unable to verify signature on transactionStatement item in "
                 "OTTransaction::VerifyTransactionReceipt.\n";
        return false;
    }
    ...
    // It's okay if some transaction #s in theMessageNym (the receipt) aren't
    // found on THE_NYM, (client-side)
    // since the last balance agreement may have cleaned them out after they
    // were recorded in
    // theMessageNym (from the transaction statement receipt).
    //
    // But I should never see transaction #s APPEAR in THE_NYM that aren't in
    // theMessageNym,
    // since a balance agreement (or a finalReceipt) can ONLY remove numbers,
    // not add them.
    //
    //
                                             ...
                                             OTIdentifier& SERVER_ID)
{
    ...
    if (strFileContents.length() < 2) {
        otErr << "OTTransaction::VerifyTransactionReceipt: Error reading file: "
    ...
    if ((NULL == pContents) ||
        (false == pContents->VerifySignature(SERVER_NYM))) {
        // receipts/SERVER_ID/USER_ID.success
        otErr << "OTTransaction::VerifyTransactionReceipt: Unable to load or "
        ...
        if (pTrans->IsAbbreviated()) {
            ...
            if (pTrans->Contains("nymboxRecord"))
                ...
                otErr << "OTTransaction::VerifyTransactionReceipt: Error "
                         ...
                         "couldn't find nymboxRecord or inboxRecord or "
                         "outboxRecord in raw contents of abbreviated "
...
{   // These are now loaded within this function, so no need to pass them in.
    ...
    OTItem* pItemWithIssuedList = NULL; // the item from that transaction that
                                        // actually has the issued list we'll be
                                        // using.

    OTItem* pResponseTransactionItem =
        NULL; // only if it's new than balance receipt does this get set, to:
    ...
    if ((NULL == pInbox) || (NULL == pOutbox)) {
        // error, return.
        otOut << "Inbox or outbox was NULL after THE_ACCOUNT.Load in "
                 "OTTransaction::VerifyBalanceReceipt.\n";
        return false;
    }
    ...
    for (int32_t i = 0; i < pInbox->GetTransactionCount(); i++) {
        ...
        // Whether pSubItem is NULL or not, pTransaction DEFINITELY exists
    ...
    , m_bCancelled(false)
{
    ...
    if (NULL != pNumList) m_Numlist = *pNumList;
}
    ...
    int64_t lTransactionNum /*=0*/)
{
    ...
    if (NULL != pTransaction) pTransaction->SetParent(theOwner);
    ...
    OTNumList* pNumList)
{
    ...
    //
    if ((NULL != pNumList) && ((OTTransaction::blank == theType) ||
...
int32_t OTTransaction::ProcessXMLNode(irr::io::IrrXMLReader*& xml)
{
    ...
    if (strNodeName.Compare("nymboxRecord") ||
        ...
        int32_t nAbbrevRetVal = OTTransaction::LoadAbbreviatedRecord(
            ...
            pNumList); // for "OTTransaction::blank" and
                       // "OTTransaction::successNotice" (Otherwise NULL.)
...
bool OTTransaction::GetSenderUserIDForDisplay(OTIdentifier& theReturnID)
{
    ...
    case OTTransaction::paymentReceipt
          ...
          // payment, it leaves a paymentReceipt...)
        {
            ...
            else if (NULL != pCronItem) // else if it is any other kind of
                                          // cron item...
            {
