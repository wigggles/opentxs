    ...
    namespace opentxs
{
                              ...
                              // or 'A' (authentication key)
    {
        ...
        for (const auto& it : m_mapCredentials) {
            ...
            const std::int32_t nTempCount = pKey->GetPublicKeysBySignature(
        ...
        , wallet_(wallet)
    {
        ...
        if (!haveChildCredential) {
            otOut << __FUNCTION__
                  << ": Creating an ed25519 child key credential." << std::endl;
            revisedParameters.setNymParameterType(NymParameterType::ED25519);
            haveChildCredential =
                !AddChildKeyCredential(revisedParameters).empty();
        }
#endif
    ...
    bool CredentialSet::CreateMasterCredential(
        const NymParameters& nymParameters)
    {
#if OT_CRYPTO_SUPPORTED_KEY_HD
        if (0 != index_) {
        ...
        const OTPassword& theExportPassword, bool bImporting)
    {
        ...
        if (m_MasterCredential->Private()) {
            ...
            // sign.)
            //
            const bool bReEncryptMaster = m_MasterCredential->ReEncryptKeys(
                theExportPassword, bImporting);
            ...
            if (bReEncryptMaster && bImporting) {
                m_MasterCredential->ReleaseSignatures(true);  // This time we'll
                                                              // sign it in
                                                              // private mode.
                bSignedMaster = m_MasterCredential->SelfSign(
                    passwordToUse, &thePWData, true);
            }
            if (!bReEncryptMaster) {
    ...
    bool CredentialSet::LoadChildKeyCredentialFromString(
        const String& strInput, const String& strSubID, const OTPassword*)
    {
    ...
    const std::string CredentialSet::GetChildCredentialIDByIndex(size_t nIndex)
        const
    {
        ...
        const CredentialIndexModeFlag mode) const
    {
        SerializedCredentialSet credSet =
            std::make_shared<proto::CredentialSet>();
    ...
    bool CredentialSet::AddContactCredential(
        const proto::ContactData& contactData)
    {
        ...
        auto version = proto::RequiredCredentialSetVersion(
            contactData.version(), version_);
        ...
        const proto::VerificationSet& verificationSet)
    {
        otOut << OT_METHOD << __FUNCTION__
              << ": Adding a verification credential." << std::endl;
    ...
    bool CredentialSet::TransportKey(Data & publicKey, OTPassword & privateKey)
        const
    {
        bool haveKey = false;
