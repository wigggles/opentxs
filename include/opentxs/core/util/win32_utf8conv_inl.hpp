...
#ifndef OPENTXS_CORE_UTIL_WIN32_UTF8CONV_INL_HPP
#define OPENTXS_CORE_UTIL_WIN32_UTF8CONV_INL_HPP
...
inline utf8_conversion_error::conversion_type utf8_conversion_error::
    conversion() const
{
    return m_conversion;
}

inline utf8_conversion_error::error_code_type utf8_conversion_error::
    error_code() const
{
    return m_error_code;
}
...
inline std::wstring UTF16FromUTF8(const std::string& utf8)
{
    ...
    const int utf16Length = ::MultiByteToWideChar(
        ...
        static_cast<int>(utf8.length()),  // length (in chars) of source UTF-8
                                          // string
    ...
    if (!::MultiByteToWideChar(
            ...
            utf8.data(),  // source UTF-8 string
            static_cast<int>(utf8.length()),  // length (in chars) of source
                                              // UTF-8 string
            &utf16[0],                        // destination buffer
            static_cast<int>(utf16.length())  // size of destination buffer, in
                                              // wchar_t's
            )) {
        // Error
        DWORD error = ::GetLastError();
        throw utf8_conversion_error(
            "Can't convert string from UTF-8 to UTF-16 (MultiByteToWideChar "
            "failed).",
            utf8_conversion_error::conversion_utf16_from_utf8,
            error);
    }
...
inline std::wstring UTF16FromUTF8(const char* utf8)
{
    ...
    if (!::MultiByteToWideChar(
            ...
            0,        // validation was done in previous call,
                      // so speed up things with default flags
            utf8,     // source UTF-8 string
            static_cast<int>(utf8Length),  // length (in chars) of source UTF-8
                                           // string
            &utf16[0],                     // destination buffer
            static_cast<int>(utf16.length())  // size of destination buffer, in
                                              // wchar_t's
...
inline std::string UTF8FromUTF16(const std::wstring& utf16)
{
    ...
    if (!::WideCharToMultiByte(
        ...
        DWORD error = ::GetLastError();
        throw utf8_conversion_error(
            "Can't convert string from UTF-16 to UTF-8 (WideCharToMultiByte "
            "failed).",
            utf8_conversion_error::conversion_utf8_from_utf16,
            error);
    }
