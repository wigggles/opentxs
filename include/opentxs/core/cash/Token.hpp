...
public:
    ...
    // server to notarize it.
    // One server might require at least 5 prototokens per withdrawal. Another
    // might require 100 because it
    // needs more security.  Another 1000.  These provide more security but they
    // also cost more in terms of
    // resources to process all those prototokens.

    EXPORT static int32_t GetMinimumPrototokenCount();

protected:
    bool m_bPasswordProtected; // this token might be encrypted to a passphrase,
                               // instead of a Nym.

    OTASCIIArmor m_ascSpendable; // This is the final, signed, unblinded token
                                 // ID, ready to be spent.
    ...
    int32_t m_nChosenIndex; // When the client submits N prototokens, the server
                            // randomly chooses one to sign.
    // (The server opens the other (N-1) prototokens to verify the amount is
    // correct and
    // that the IDs are random enough.)
    // Expiration dates are necessary because otherwise the spent token database
    // must be stored
    // forever. This may be useful in some applications, but in most, a 1-year
    // or 1-month expiration
    // date will be perfectly fine, especially with auto-exchanges performed by
    // the wallet. Suddenly
    // it becomes much more feasible a proposition to effectively run a token
...
public:
    ...
    // LucreTokens,
    // and other types of tokens, dynamically, without having to know beforehand
    // which
    ...
    inline void SetSavePrivateKeys()
    {
        m_bSavePrivateKeys = true;
    }

    // When you send a token to the server, you must decrypt it from your own
    // key,
    // and re-encrypt it to the server key, before sending. Use this function to
    // do so.
    // In the case of exporting a token from a purse, you could create a dummy
    // Nym, embed
    // it inside the purse, and then reassign ownership of each token to that
    // token as you
    // push them into that purse.
    // From there, you can hand someone the purse, and password-protect it, if
    // you like.
    //
    EXPORT bool ReassignOwnership(OTNym_or_SymmetricKey& oldOwner,
                                  OTNym_or_SymmetricKey& newOwner);

    inline const OTASCIIArmor& GetSpendable() const
    {
        return m_ascSpendable;
    }
    ...
    inline OTToken::tokenState GetState() const
    {
        return m_State;
    }
...
protected:
    EXPORT virtual bool GenerateTokenRequest(
        ...
        int32_t nTokenCount = OTToken::GetMinimumPrototokenCount()) = 0;

public:
    EXPORT static OTToken* InstantiateAndGenerateTokenRequest(
    ...
    inline int32_t GetSeries() const
    {
        return m_nSeries;
    }
    ...
    // verifies
    // when the server uses the private verify info for THAT denomination. So if
    // you set
    // the denomination here wrong, all that does is cause the server to try to
    // verify it
    // with the wrong key. If the proto-token was generated for a different
    // denomination,
    // then it cannot verify.
    // So this value is only here to help you make sure to ask the Mint to use
    // the right
    // key when verifying the token. And this only works because we have a
    // specific set of
    // denominations for each digital asset, each with its own key pair in the
    // Mint.
    inline int64_t GetDenomination() const
    {
        return m_lDenomination;
    }
    inline void SetDenomination(int64_t lVal)
    {
        m_lDenomination = lVal;
    }
