...
private:
    ...
    OTString m_strName; // Used by the wallet so the nym is easily identified by
                        // the user
                        // The internals, and server, prefer nymID to name.
    OTString m_strNymfile; // This contains the request numbers and other user
                           // acct info. XML.
    // Client-side only, since the server uses nymID for filenames
    OTString m_strCertfile; // Filename for pem file that contains the x509
    ...
    OTASCIIArmor m_ascCert; // Just the ascii-armor portion without BEGIN and
                            // END
    OTString m_strSourceForNymID; // Hash this to form the NymID. Can be a
                                  // public key, or a URL, or DN info from a
                                  // cert, etc.
    OTString m_strAltLocation; // If the Nym's credential IDs cannot be directly
...
private:
    ...
    // Nymbox (OTIdentifier) mapped by ServerID (std::string)
public:
    // This value is only updated on client side, when the actual latest
    // nymbox has been downloaded.
    EXPORT bool GetNymboxHash(const std::string& server_id,
                              OTIdentifier& theOutput) const; // client-side
    EXPORT bool SetNymboxHash(const std::string& server_id,
                              const OTIdentifier& theInput); // client-side
    ...
    // nymbox
    // because it's old.
    EXPORT bool GetRecentHash(const std::string& server_id,
                              OTIdentifier& theOutput) const; // client-side
    EXPORT bool SetRecentHash(const std::string& server_id,
                              const OTIdentifier& theInput); // client-side
    ...
    inline OTString& GetNymName()
    {
        return m_strName;
    }
    inline void SetNymName(const OTString& strName)
    {
        m_strName = strName;
    }
    ...
    EXPORT OTPseudonym(const OTString& name, const OTString& filename,
    ...
    EXPORT void GetIdentifier(OTIdentifier& theIdentifier) const; // BINARY
                                                                  // VERSION
    ...
    EXPORT void OnUpdateRequestNum(OTPseudonym& SIGNER_NYM,
                                   ...
                                   int64_t lNewRequestNumber); // if the server
                                                               // sends us a
                                                               // @getRequest
    EXPORT bool GetCurrentRequestNum(const OTString& strServerID,
                                     int64_t& lReqNum); // get the current
                                                        // request number for
                                                        // the serverID

    EXPORT bool GetHighestNum(const OTString& strServerID,
                              int64_t& lHighestNum); // get the last/current
                                                     // highest transaction
    ...
    EXPORT bool UnRegisterAtServer(const OTString& strServerID); // Removes the
                                                                 ...
                                                                 // it was there
                                                                 // before.
    EXPORT bool IsRegisteredAtServer(const OTString& strServerID); // You can't
                                                                   // go using a
                                                                   // Nym at a
                                                                   // certain
                                                                   // server, if
    ...
    EXPORT bool ResyncWithServer(OTLedger& theNymbox,
                                 OTPseudonym& theMessageNym);
    // HIGH LEVEL:
    EXPORT bool AddTransactionNum(
        OTPseudonym& SIGNER_NYM, const OTString& strServerID, int64_t lTransNum,
        bool bSave); // We have received a new trans num from server. Store it.
    ...
    bool RemoveTentativeNum(OTPseudonym& SIGNER_NYM,
                            const OTString& strServerID,
    ...
    EXPORT bool RemoveAcknowledgedNum(OTPseudonym& SIGNER_NYM,
                                      ...
                                      bool bSave); // Used on both client and
                                                   // server sides for
                                                   // optimization.
    EXPORT bool VerifyIssuedNum(const OTString& strServerID,
                                const int64_t& lTransNum); // verify user is
                                                           // still responsible
                                                           // for (signed for) a
                                                           // certain trans#
                                                           // that was previous
                                                           ...
                                                           // accepted receipt
                                                           // through inbox.)
    EXPORT bool VerifyTransactionNum(const OTString& strServerID,
    ...
    EXPORT bool VerifyTentativeNum(const OTString& strServerID,
                                   const int64_t& lTransNum); // Client-side
                                                              // verifies that
                                                              // it actually
                                                              // tried to sign
                                                              // for this number
    ...
    EXPORT bool VerifyAcknowledgedNum(const OTString& strServerID,
                                      const int64_t& lRequestNum); // Client
                                                                   // verifies
                                                                   // it has
                                                                   // already
                                                                   // seen a
    ...
    EXPORT int64_t
    GetIssuedNum(const OTIdentifier& theServerID, int32_t nIndex); // index

    EXPORT bool AddIssuedNum(const OTString& strServerID,
                             const int64_t& lTransNum); // doesn't save

    EXPORT bool RemoveIssuedNum(OTPseudonym& SIGNER_NYM,
                                const OTString& strServerID,
                                const int64_t& lTransNum); // saves
    EXPORT bool RemoveIssuedNum(const OTString& strServerID,
                                const int64_t& lTransNum); // doesn't save
    // These functions are for transaction numbers that I still have available
    // to use.
    //
    EXPORT int32_t
    GetTransactionNumCount(const OTIdentifier& theServerID); // count
    EXPORT int64_t
    GetTransactionNum(const OTIdentifier& theServerID, int32_t nIndex); // index
    ...
    EXPORT bool RemoveTransactionNum(OTPseudonym& SIGNER_NYM,
                                     ...
                                     const int64_t& lTransNum); // server
                                                                ...
                                                                // nym file.
                                                                // Saves.
    EXPORT bool RemoveTransactionNum(const OTString& strServerID,
                                     const int64_t& lTransNum); // doesn't save.
    ...
    //
    EXPORT int32_t
    GetTentativeNumCount(const OTIdentifier& theServerID); // count
    EXPORT int64_t
    GetTentativeNum(const OTIdentifier& theServerID, int32_t nIndex); // index

    EXPORT bool AddTentativeNum(const OTString& strServerID,
                                const int64_t& lTransNum); // doesn't save

    EXPORT bool RemoveTentativeNum(OTPseudonym& SIGNER_NYM,
                                   const OTString& strServerID,
                                   const int64_t& lTransNum);
    EXPORT bool RemoveTentativeNum(const OTString& strServerID,
                                   const int64_t& lTransNum); // doesn't save.
    ...
    //
    EXPORT int32_t
    GetAcknowledgedNumCount(const OTIdentifier& theServerID); // count
    EXPORT int64_t GetAcknowledgedNum(const OTIdentifier& theServerID,
                                      int32_t nIndex); // index

    EXPORT bool AddAcknowledgedNum(const OTString& strServerID,
                                   const int64_t& lRequestNum); // doesn't save

    EXPORT bool RemoveAcknowledgedNum(OTPseudonym& SIGNER_NYM,
                                      const OTString& strServerID,
                                      const int64_t& lRequestNum);
    EXPORT bool RemoveAcknowledgedNum(const OTString& strServerID,
    ...
    //
    EXPORT bool VerifyGenericNum(mapOfTransNums& THE_MAP,
                                 const OTString& strServerID,
                                 const int64_t& lTransNum);
    ...
    EXPORT int32_t GetGenericNumCount(mapOfTransNums& THE_MAP,
                                      const OTIdentifier& theServerID);
    EXPORT int64_t GetGenericNum(mapOfTransNums& THE_MAP,
                                 const OTIdentifier& theServerID,
                                 int32_t nIndex);
    ...
    EXPORT int32_t
    GetMailCount(); // How many mail messages does this Nym currently store?
    EXPORT OTMessage* GetMailByIndex(const int32_t nIndex); // Get a specific
                                                            // piece of mail, at
                                                            // a specific index.
    ...
    EXPORT void AddOutmail(OTMessage& theMessage); // a mail message is the
                                                   ...
                                                   // this Nym sent.
    EXPORT int32_t GetOutmailCount(); // How many outmail messages does this Nym
                                      // currently store?
    EXPORT OTMessage* GetOutmailByIndex(const int32_t nIndex); // Get a specific
                                                               // piece of
                                                               // outmail, at a
                                                               // specific
                                                               // index.
    ...
    EXPORT void AddOutpayments(OTMessage& theMessage); // a payments message is
                                                       ...
                                                       // that this Nym sent.
    EXPORT int32_t GetOutpaymentsCount(); // How many outpayments messages does
                                          // this Nym currently store?
    EXPORT OTMessage* GetOutpaymentsByIndex(
        const int32_t nIndex); // Get a specific piece of outpayments, at a
                               // specific index.
