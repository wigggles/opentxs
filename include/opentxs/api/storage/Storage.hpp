...
public:
    ...
    EXPORT virtual std::string AccountAlias(
        const Identifier& accountID) const = 0;
    EXPORT virtual ObjectList AccountList() const = 0;
    EXPORT virtual OTUnitID AccountContract(
        const Identifier& accountID) const = 0;
    EXPORT virtual OTNymID AccountIssuer(const Identifier& accountID) const = 0;
    EXPORT virtual OTNymID AccountOwner(const Identifier& accountID) const = 0;
    EXPORT virtual OTServerID AccountServer(
        const Identifier& accountID) const = 0;
    EXPORT virtual OTNymID AccountSigner(const Identifier& accountID) const = 0;
    EXPORT virtual proto::ContactItemType AccountUnit(
        const Identifier& accountID) const = 0;
    EXPORT virtual std::set<OTIdentifier> AccountsByContract(
        const identifier::UnitDefinition& contract) const = 0;
    EXPORT virtual std::set<OTIdentifier> AccountsByIssuer(
        const identifier::Nym& issuerNym) const = 0;
    EXPORT virtual std::set<OTIdentifier> AccountsByOwner(
        const identifier::Nym& ownerNym) const = 0;
    EXPORT virtual std::set<OTIdentifier> AccountsByServer(
        const identifier::Server& server) const = 0;
    EXPORT virtual std::set<OTIdentifier> AccountsByUnit(
        const proto::ContactItemType unit) const = 0;
    EXPORT virtual OTIdentifier Bip47AddressToChannel(
        const identifier::Nym& nymID,
        const std::string& address) const = 0;
    EXPORT virtual proto::ContactItemType Bip47Chain(
        const identifier::Nym& nymID,
        const Identifier& channelID) const = 0;
    EXPORT virtual Bip47ChannelList Bip47ChannelsByContact(
        const identifier::Nym& nymID,
        const Identifier& contactID) const = 0;
    EXPORT virtual Bip47ChannelList Bip47ChannelsByChain(
        const identifier::Nym& nymID,
        const proto::ContactItemType chain) const = 0;
    EXPORT virtual Bip47ChannelList Bip47ChannelsByLocalPaymentCode(
        const identifier::Nym& nymID,
        const std::string& code) const = 0;
    EXPORT virtual Bip47ChannelList Bip47ChannelsByRemotePaymentCode(
        const identifier::Nym& nymID,
        const std::string& code) const = 0;
    EXPORT virtual ObjectList Bip47ChannelsList(
        const identifier::Nym& nymID) const = 0;
    EXPORT virtual OTIdentifier Bip47Contact(
        const identifier::Nym& nymID,
        const Identifier& channelID) const = 0;
    EXPORT virtual std::string Bip47LocalPaymentCode(
        const identifier::Nym& nymID,
        const Identifier& channelID) const = 0;
    ...
    EXPORT virtual proto::ContactItemType BlockchainAccountType(
        const std::string& nymID,
        const std::string& accountID) const = 0;
    EXPORT virtual std::string BlockchainAddressOwner(
...
#endif
    EXPORT virtual std::string ContactAlias(const std::string& id) const = 0;
    EXPORT virtual ObjectList ContactList() const = 0;
    EXPORT virtual ObjectList ContextList(const std::string& nymID) const = 0;
    EXPORT virtual std::string ContactOwnerNym(
        const std::string& nymID) const = 0;
    EXPORT virtual void ContactSaveIndices() const = 0;
    EXPORT virtual VersionNumber ContactUpgradeLevel() const = 0;
    EXPORT virtual bool CreateThread(
        const std::string& nymID,
        const std::string& threadID,
        const std::set<std::string>& participants) const = 0;
    EXPORT virtual bool DeleteAccount(const std::string& id) const = 0;
    EXPORT virtual std::string DefaultSeed() const = 0;
    EXPORT virtual bool DeleteContact(const std::string& id) const = 0;
    EXPORT virtual bool DeletePaymentWorkflow(
        const std::string& nymID,
        const std::string& workflowID) const = 0;
    EXPORT virtual std::uint32_t HashType() const = 0;
    EXPORT virtual ObjectList IssuerList(const std::string& nymID) const = 0;
...
#if OT_CASH
    EXPORT virtual bool MarkTokenSpent(
        const identifier::Server& notary,
        const identifier::UnitDefinition& unit,
        const std::uint64_t series,
        const std::string& key) const = 0;
#endif
    EXPORT virtual bool MoveThreadItem(
        const std::string& nymId,
        const std::string& fromThreadID,
        const std::string& toThreadID,
        const std::string& itemID) const = 0;
    EXPORT virtual ObjectList NymBoxList(
        const std::string& nymID,
        const StorageBox box) const = 0;
    EXPORT virtual ObjectList NymList() const = 0;
    EXPORT virtual ObjectList PaymentWorkflowList(
        const std::string& nymID) const = 0;
    EXPORT virtual std::string PaymentWorkflowLookup(
        const std::string& nymID,
        const std::string& sourceID) const = 0;
    EXPORT virtual std::set<std::string> PaymentWorkflowsByAccount(
        const std::string& nymID,
        const std::string& accountID) const = 0;
    EXPORT virtual std::set<std::string> PaymentWorkflowsByState(
        const std::string& nymID,
        const proto::PaymentWorkflowType type,
        const proto::PaymentWorkflowState state) const = 0;
    EXPORT virtual std::set<std::string> PaymentWorkflowsByUnit(
        const std::string& nymID,
    ...
    EXPORT virtual std::
        ...
        PaymentWorkflowState(
            const std::string& nymID,
            const std::string& workflowID) const = 0;
    EXPORT virtual bool RelabelThread(
        const std::string& threadID,
        const std::string& label) const = 0;
    EXPORT virtual bool RemoveNymBoxItem(
        const std::string& nymID,
        const StorageBox box,
        const std::string& itemID) const = 0;
    EXPORT virtual bool RemoveServer(const std::string& id) const = 0;
    EXPORT virtual bool RemoveThreadItem(
        const identifier::Nym& nym,
    ...
    EXPORT virtual void RunGC() const = 0;
    EXPORT virtual std::string ServerAlias(const std::string& id) const = 0;
    EXPORT virtual ObjectList ServerList() const = 0;
    EXPORT virtual ObjectList SeedList() const = 0;
    EXPORT virtual bool SetAccountAlias(
        const std::string& id,
        const std::string& alias) const = 0;
    EXPORT virtual bool SetContactAlias(
        const std::string& id,
        const std::string& alias) const = 0;
    EXPORT virtual bool SetDefaultSeed(const std::string& id) const = 0;
    EXPORT virtual bool SetNymAlias(
        const std::string& id,
        const std::string& alias) const = 0;
    EXPORT virtual bool SetPeerRequestTime(
        const std::string& nymID,
        const std::string& id,
        const StorageBox box) const = 0;
    EXPORT virtual bool SetReadState(
        const std::string& nymId,
        const std::string& threadId,
        const std::string& itemId,
        const bool unread) const = 0;
    EXPORT virtual bool SetSeedAlias(
        const std::string& id,
        const std::string& alias) const = 0;
    EXPORT virtual bool SetServerAlias(
        const std::string& id,
        const std::string& alias) const = 0;
    EXPORT virtual bool SetThreadAlias(
        const std::string& nymId,
        const std::string& threadId,
        const std::string& alias) const = 0;
    ...
    EXPORT virtual bool Store(
        ...
        const std::string& alias = std::string("")) const = 0;
    EXPORT virtual ObjectList ThreadList(
        const std::string& nymID,
        const bool unreadOnly) const = 0;
    EXPORT virtual std::string ThreadAlias(
        const std::string& nymID,
        const std::string& threadID) const = 0;
    EXPORT virtual std::string UnitDefinitionAlias(
        const std::string& id) const = 0;
    EXPORT virtual ObjectList UnitDefinitionList() const = 0;
    EXPORT virtual std::size_t UnreadCount(
        const std::string& nymId,
        const std::string& threadId) const = 0;
    EXPORT virtual void UpgradeNyms() = 0;

    EXPORT virtual ~Storage() = default;
