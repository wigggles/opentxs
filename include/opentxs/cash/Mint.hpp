...
protected:
    ...
    int32_t m_nDenominationCount{0}; // How many denominations of the currency are
                                  // issued by this Mint?
                                  // (Each requires its own key pair.)

    bool m_bSavePrivateKeys{false}; // Determines whether it serializes private keys
                             // (no if false).
    ...
    int32_t m_nSeries{0}; // Each series of the mint has a valid from and to date.
                       // Series should rotate.
    // (That is, the new one should be introduced halfway through the validity
    // period
    // of the current one, and so on...)

    time64_t m_VALID_FROM{0}; // All tokens generated by this series will have the
                           // same From and To dates.
    time64_t m_VALID_TO{0}; // This way they cannot be tracked by expiration date.

    time64_t m_EXPIRATION{0}; // The Mint itself expires before the tokens do. (As
                           // the new series rotates in...)

    Identifier m_CashAccountID; // The Account ID for the cash reserve
                                // account. (Pointer below.)

    Account* m_pReserveAcct{nullptr}; // When you withdraw cash, the Mint removes X
