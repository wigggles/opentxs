...
protected:
    ...
    bool m_bSavePrivateKeys; // Determines whether it serializes private keys
                             // (no if false).
    ...
    int32_t m_nSeries; // Each series of the mint has a valid from and to date.
                       // Series should rotate.
    // (That is, the new one should be introduced halfway through the validity
    // period
    // of the current one, and so on...)

    time64_t m_VALID_FROM; // All tokens generated by this series will have the
