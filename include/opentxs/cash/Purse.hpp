...
// The recipient will already know to use his own private key to open the purse,
// and then he will immediately
// open it, redeem the coins, and store the replacements again encrypted to his
// own key, until he spends them
// again to someone else, when he will also know to encrypt the purse to THEIR
...
protected:
    ...
    // Key.
    // The symmetric key is used to store the actual key for
    // encrypting/decrypting the tokens in this purse.
    // Whereas the master key is used for retrieving the passphrase to use for
    // unlocking the symmetric key.
    // The passphrase in question is actually a random number stored inside the
    // master key, inside its own
    // internal symmetric key. In order to unlock it, OTCachedKey may
    // occasionally ask the user to enter a
    // passphrase, which is used to derived a key to unlock it. This key may
    // then be cached in memory by
    // OTCachedKey until a timeout, and later be zapped by a thread for that
    // purpose.
    bool m_bIsNymIDIncluded; // It's possible to use a purse WITHOUT attaching
                             // the relevant NymID. (The holder of the purse
                             // just has to "know" what the correct NymID is, or
                             // it won't work.) This bool tells us whether the
...
public:
    ...
    //
    EXPORT bool GenerateInternalKey(); // Create internal symmetric key for
    ...
    // tokens within.
    // Therefore this will never be as accurate as individually examining those
    // tokens...
    //
