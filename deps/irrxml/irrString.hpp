...
public:
        ...
        , used(0)
    {
...

        {
            ...
            if (lenght >= used || lenght >= allocated)
                abort();  // must not overflow.
        }

        array = new T[used];

        for (s32 l = 0; l < (used - 1); ++l) array[l] = (T)c[l];
    ...
    string<T>& operator=(const B* c)
    {
        ...
        if ((void*)c == (void*)array) return *this;
        ...
        // we'll take the old string for a while, because the new string could
        // be a part of the current string.
        T* oldArray = array;
    ...
    void make_lower()
    {
        ...
        for (s32 i = 0; i < used; ++i) {
            if (array[i] >= A && array[i] <= Z) array[i] += diff;
        }
    }
    ...
    bool equals_ignore_case(const string<T>& other) const
    {
        for (s32 i = 0; array[i] && other[i]; ++i)
            if (toLower(array[i]) != toLower(other[i])) return false;
    ...
    {
        if (used + 1 > allocated) reallocate((s32)used + 1);
    ...
    void append(const string<T>& other)
    {
        ...
        if (used + len + 1 > allocated) reallocate((s32)used + (s32)len + 1);
   ...
   */
    void append(const string<T>& other, s32 length)
    {
        ...
        if (used + len > allocated) reallocate((s32)used + (s32)len);
    ...
    should find the first occurance of 'a' or 'b', this parameter should be
    "ab". \param count: Amount of characters in the list. Ususally, this should
    be strlen(ofParameter1) \return Returns position where one of the character
    ...
    /** \param c: List of characters not to find. For example if the method
     should find the first occurance of a character not 'a' or 'b', this
    parameter should be "ab". \param count: Amount of characters in the list.
    Ususally, this should be strlen(ofParameter1) \return Returns position where
    the character has been found, or -1 if not found. */
    template <class B>
    ...
    {
        for (s32 i = 0; i < used; ++i)
            if (array[i] == toReplace) array[i] = replaceWith;
    }
