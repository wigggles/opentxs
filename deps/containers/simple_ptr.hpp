...
//   reference the object are cleared (by deletion, manual clearing or
//   reassignment).

//   Also, the simple pointer cannot contain a reference to a shared null
//   pointer
//   (which occurs as a side-effect of clearing a multiply referenced object in
//   the smart_ptr classes). Which means that if you have a null simple_ptr,
//   then
//   the assignment of any other null simple_ptr will NOT reassign the reference
//   of
//   any other simple_ptr. Hence, the simple_ptr class acts a little more like a
//   normal pointer (with fewer side effects), with the added bonus of
//   containment.
...
//   have two simple_ptr's of different template types, both pointing to the
//   same
...
//   - simple_ptr        for simple types and classes which have copy
//   constructors
//   - simple_ptr_clone  for polymorphic class hierarchies which are copied
...
public:
    ...
    // constructor form - must be called in the form smart_ptr_base<type> x(new
    // type(args))
    ...
    // assignment operator - required, else the output of GCC suffers
    // segmentation faults
    simple_ptr_base<T, C>& operator=(const simple_ptr_base<T, C>& r);

    // destructor decrements the reference count and delete only when the last
    // reference is destroyed
    ~simple_ptr_base(void);

    //////////////////////////////////////////////////////////////////////////////
    // logical tests to see if there is anything contained in the pointer since
    // it can be null
    ...
    // used as a prefix to a member access to the contained object e.g.
    // p1->print() calls T::print()
    T* operator->(void);
    const T* operator->(void)const;
    ...
    // Note: the object must be allocated *by the user* with new
    // Warning: it is very easy to break the memory management with this
    // operation
    void set(T* data = 0);
    // get the pointer
    T* pointer(void);
    ...
    // test whether two pointers point to the same object(known as aliasing the
    // object)
    ...
    // clear the reference to the object, but only delete the object if there
    // are no
    // other references to that object. Hence, this does not affect other
    // pointers
    // that are pointing to the same object.
    void clear(void);

    // This is just an alias of the clear() function, provided for completeness
    // of
    ...
    // create a pointer containing a *copy* of the object using the template
    // parameter C
    // this copy is taken because the pointer class maintains a dynamically
    // allocated object
    ...
    // make this pointer unique with respect to any other references to the same
    // object
    // if this pointer is already unique, it does nothing - otherwise it copies
    // the object
    void make_unique(void);

    // make this pointer a unique copy of the parameter
    // useful for expressions like p1.copy(p2) which makes p1 a pointer to a
    // unique copy of the contents of p2
...
public:
    simple_ptr(void) {}
    explicit simple_ptr(const T& data)
    ...
    // moved from base class for two main reasons, though the second is a
    // feature of the first:

    // 1. GCC cannot cast the previous base result of simple_ptr_base<T2,
    // constructor_copy<T> >
    //    as a simple_ptr<T2> even though it used to look like a duck and quack
    //    like a duck.
    //    I think it was really complaining that the copy class was not
    //    guaranteed to be the same.

    // 2. Within the cast routines, one pointer type tried accessing private
    // data of the other
    //    pointer type and even though they are really the same type, was not
    //    allowed. Because
    //    of this, the "private" function _make_alias is utilised to get the
    //    same result.

    // By having the cast functions in each derived class, you are guaranteed to
    // use the same
    // copy class - no question. GCC is ok with this.

    template <typename T2>
    simple_ptr<T2> dyn_cast(void) const;
    template <typename T2>
    simple_ptr<T2> stat_cast(void) const;
    template <typename T2>
    simple_ptr<T2> cast(void) const;
#endif
};
...
public:
    ...
    explicit simple_ptr_clone(const T& data)
    ...
    template <typename T2>
    simple_ptr_clone<T2> dyn_cast(void) const;
    template <typename T2>
    simple_ptr_clone<T2> stat_cast(void) const;
    template <typename T2>
    simple_ptr_clone<T2> cast(void) const;
#endif
};
...
public:
    ...
    // moved from base class - see simple_ptr above
    template <typename T2>
    simple_ptr_nocopy<T2> dyn_cast(void) const;
    template <typename T2>
    simple_ptr_nocopy<T2> stat_cast(void) const;
    template <typename T2>
    simple_ptr_nocopy<T2> cast(void) const;
#endif
};
