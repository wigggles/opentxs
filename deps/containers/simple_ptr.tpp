...
// constructor form - must be called in the form simple_ptr<type> x(new
// type(args))
template <typename T, typename C>
...
// assignment operator - required, else the output of GCC suffers segmentation
// faults
template <typename T, typename C>
simple_ptr_base<T, C>& simple_ptr_base<T, C>::operator=(
    const simple_ptr_base<T, C>& r)
{
    alias(r);
    return *this;
}
...
//////////////////////////////////////////////////////////////////////////////
// logical tests to see if there is anything contained in the pointer since it
// can be null
...
T& simple_ptr_base<T, C>::operator*(void)
{
    if (!m_pointer)
        throw null_dereference(
            "null pointer dereferenced in simple_ptr::operator*");
    return *m_pointer;
}
...
T& simple_ptr_base<T, C>::value(void)
{
    if (!m_pointer)
        throw null_dereference(
            "null pointer dereferenced in simple_ptr::value");
    return *m_pointer;
}
...
void simple_ptr_base<T, C>::set(T* data)
{
    ...
    if (count == 1) {
        // The count member can be reused, but the current pointer should be
        // deleted
        delete m_pointer;
    } else if (!count) {
        // The count member is indicating that it will be destructed, so assign
        // a new one
        // for the given object. This might only happen when a pointer is being
        // destructed
        // and the contained object makes a copy of the pointer and then sets it
        // to something.
        // Very tenuous I know, but still it's possible. Whatever, the object
        // should NOT
        // be deleted, as it is probably in the middle of destruction already.
        m_count = new unsigned(1);
    } else {
        // Another pointer holds a reference to the current object, so just
        // decrement
...
void simple_ptr_base<T, C>::increment(void)
{
    ...
    // as the object pointed to by this pointer might be in the middle of
    // destruction
    // This can happen if a callback within the destructor of an object held by
    // this
    // pointer makes a copy of it's container pointer (this).
    // The result of incrementing will be that the object gets
    // double-deleted/destructed
    // which can lead to all sorts of errors as destructors are generally not
    // recursive
    // A similar check for decrementing is made below
    // Note that objects are deleted BEFORE the count container is deleted, so
    // the count
