...
TEST(Test_AccountList, testList)
{
    ...
    OTIdentifier AliceAccountID = OT::App().Blockchain().NewAccount(
        Identifier(AliceNymID), BlockchainAccountType::BIP44, proto::CITEMTYPE_BTC);
    std::cout << "Created Alice's Account: " << String(AliceAccountID).Get()
              << " !!\n";

    OTIdentifier AliceAccountID2 = OT::App().Blockchain().NewAccount(
        Identifier(AliceNymID2), BlockchainAccountType::BIP32, proto::CITEMTYPE_BTC);
    std::cout << "Created Alice's 2nd Account: "
              << String(AliceAccountID2).Get() << " !!\n";

    const std::set<OTIdentifier> as =
      opentxs::OT::App().Blockchain().AccountList(Identifier(AliceNymID), proto::CITEMTYPE_BTC);
    const std::set<OTIdentifier> bs =
      opentxs::OT::App().Blockchain().AccountList(Identifier(BobNymID), proto::CITEMTYPE_BTC);
    EXPECT_EQ(1, as.size());
    ASSERT_EQ(0, bs.size());   

    OTIdentifier BobAccountID = OT::App().Blockchain().NewAccount(
        Identifier(BobNymID), BlockchainAccountType::BIP44, proto::CITEMTYPE_BTC);
    ...
    OTIdentifier AliceLTCAccountID = OT::App().Blockchain().NewAccount(
        Identifier(AliceNymID), BlockchainAccountType::BIP44, proto::CITEMTYPE_LTC);

    const std::set<OTIdentifier> als =
        opentxs::OT::App().Blockchain().AccountList(Identifier(AliceNymID), proto::CITEMTYPE_LTC);
